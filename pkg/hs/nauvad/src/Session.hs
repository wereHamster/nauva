...
sessionStart :: Session -> (Stream -> String -> IO ()) -> String -> IO ([Load], [FilePath])
sessionStart Session{..} echo cmd = do
    modifyVar_ running $ const $ return False
    ...
    -- start the new
    echo Stdout $ "Loading " ++ cmd ++ " ..."
    (v, messages) <- startGhci cmd Nothing $ echo
...
sessionRestart :: Session -> (Stream -> String -> IO ()) -> IO ([Load], [FilePath])
sessionRestart session@Session{..} echo = do
    Just cmd <- readIORef command
    sessionStart session echo cmd
...
--   added back in.
sessionReload :: Session -> (Stream -> String -> IO ()) -> IO ([Load], [FilePath])
sessionReload session@Session{..} echo = do
    ...
    if stuck then sessionRestart session echo else do
        -- actually reload
        Just ghci <- readIORef ghci
